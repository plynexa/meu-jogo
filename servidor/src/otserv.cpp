...
ServiceManager* services)
{
	...
	#endif
	g_RSA = RSA_new();
	BIGNUM *rsaP = NULL, *rsaQ = NULL, *rsaD = NULL, *rsaN = NULL, *rsaE = NULL;
	bool keyReady = g_RSA &&
		BN_dec2bn(&rsaP, g_config.getString(ConfigManager::RSA_PRIME1).c_str()) > 0 &&
		BN_dec2bn(&rsaQ, g_config.getString(ConfigManager::RSA_PRIME2).c_str()) > 0 &&
		BN_dec2bn(&rsaD, g_config.getString(ConfigManager::RSA_PRIVATE).c_str()) > 0 &&
		BN_dec2bn(&rsaN, g_config.getString(ConfigManager::RSA_MODULUS).c_str()) > 0 &&
		BN_dec2bn(&rsaE, g_config.getString(ConfigManager::RSA_PUBLIC).c_str()) > 0;
	if(keyReady && RSA_set0_key(g_RSA, rsaN, rsaE, rsaD) == 1)
		rsaN = rsaE = rsaD = NULL;
	else
		keyReady = false;
	if(keyReady && RSA_set0_factors(g_RSA, rsaP, rsaQ) == 1)
		rsaP = rsaQ = NULL;
	else
		keyReady = false;
	BN_clear_free(rsaP); BN_clear_free(rsaQ); BN_clear_free(rsaD);
	BN_clear_free(rsaN); BN_clear_free(rsaE);
	if(keyReady && RSA_check_key(g_RSA) == 1)
	{
		// OpenSSL derives CRT parameters when needed; RSA internals are opaque.
	}
	else
	{
